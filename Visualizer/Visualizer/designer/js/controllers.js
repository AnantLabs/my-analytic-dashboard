	...
	$http
			...
			.success(
					function(data) {
						...
						for ( var i = 0; i < data.ShortNameDefinition.ShortNameDefs.ShortNameDef.length; i++) {
							$scope.attrs
									.push(data.ShortNameDefinition.ShortNameDefs.ShortNameDef[i].ShortName);
						}
						;
						$rootScope.attrs = $scope.attrs;
					});}
...
function HomeCtrl($http, $scope, $rootScope) {
	...
	$scope.showExpProcessingPopup = function(appName){debugger;
...
function createURL(addNewInd) {
	...
	if (addNewInd) {
		Metamodel = 'Template_Default';
		window.location = '#/canvas-nb-main/' + Metamodel + "/" 
				+ "?AddNew&NewName=" + $("#model").get(0).value;
	} else {
		redirectUrl = '#/canvas-nb-main/' + Metamodel + "/" ;
...
function LeftControlPalette($scope, $http,$rootScope) {
	...
	// for the properties palette in the left side
	...
	$http
			.get('data/canvas-control-palette.json')
			.success(
	...
	$scope.accordClick = function() {
		$http
				.get(
						$rootScope.listURI.ApplicationURL
								+ $rootScope.listURI.GetAttributesFullServiceURI
								+ '/' + $rootScope.setType + '/json')
				.success(
						function(data) {
							$scope.attrsJson = data.ShortNameDefinition.ShortNameDefs.ShortNameDef;
							for ( var i = 0; i < data.ShortNameDefinition.ShortNameDefs.ShortNameDef.length; i++) {
			...
			$http(
					{
						...
						data : $rootScope.accessControldata												
					}).success(function(data) {	
						var savedtoken = data;
					}).error(function(data) {
						var savedtoken = data;
...
function NewattCtrl($scope, $routeParams, $rootScope, $http) {
	...
	$scope.submitNewAttr = function(action) {
		...
		case "back":
			skipHttp = true;
			window.location = "#/canvas-nb-main/" + $routeParams.screenId + "/"
...
var ModalInstanceCtrl = function($scope, $modalInstance, field, $rootScope,
		$http) {
